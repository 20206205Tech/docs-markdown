...
* **Thao tác khởi tạo của người dùng và các xử lý bước đầu:** Luồng bắt đầu khi người dùng gửi HTTP POST request (`/code-conversation-service/shares/:id/revoke`) để yêu cầu thu hồi chia sẻ. Conversation Service tiếp nhận, thực hiện một giao dịch nguyên tử (Atomic Transaction) trên Conversation DB để cập nhật trạng thái bản ghi Share (`isActive=false`) đồng thời chèn (insert) sự kiện `ShareRevokedEvent` vào bảng Outbox. Ngay sau khi transaction hoàn tất, dịch vụ phản hồi `HTTP 200 OK` cho người dùng để xác nhận thành công mà không phải chờ các xử lý phía sau.
* **Xử lý gửi nhận thông điệp bất đồng bộ (Outbox Relay, Message Broker):** Để đảm bảo không mất mát thông điệp (Reliable Messaging), hệ thống áp dụng Transactional Outbox Pattern. Một tiến trình nền (Cron job OutboxRelay) định kỳ truy vấn Conversation DB để lấy các sự kiện Outbox đang ở trạng thái `PENDING`. Những sự kiện này sau đó được đẩy (publish) vào hàng đợi `share_revoked` trên RabbitMQ. Sau khi RabbitMQ xác nhận (Ack) đã nhận thông điệp, Conversation Service sẽ cập nhật trạng thái bản ghi trong bảng Outbox thành `DONE`.
* **Giao tiếp với bên thứ ba / Cổng dịch vụ ngoài:** Ở phía consumer, Chatbot Service nhận (`Consume`) thông điệp `ShareRevokedEvent` từ RabbitMQ. Nhiệm vụ đầu tiên của dịch vụ này là gọi sang hệ thống lưu trữ bên ngoài (R2 Object Storage) để thực hiện lệnh xóa file dữ liệu JSONL liên quan đến phiên chia sẻ bị thu hồi.
* **Quy trình đồng bộ dữ liệu ở các Service tiêu thụ khác:** Sau khi nhận được phản hồi xóa file thành công từ R2, Chatbot Service tiến hành cập nhật trạng thái bản ghi Share trong Chatbot DB (`is_revoked=True`). Thao tác này có sử dụng thông tin phiên bản (`version`) để đảm bảo tính nhất quán của dữ liệu (Optimistic Concurrency Control). Cuối cùng, Chatbot Service gửi tín hiệu Acknowledge (Ack) về cho RabbitMQ để đánh dấu thông điệp đã được xử lý hoàn toàn và có thể xóa khỏi hàng đợi.
